...
public final class DeletedHtmlRowGenerator {
    ...
    public static <T> ImmutableList<Row> createDeletedRows(final T object, final int indent) {
        final Function<RowAttributes, Row> toDeletedRowFunc = (rowAttributes ->
                new DeletedRow(rowAttributes.getIndent(), rowAttributes.getProperty(), rowAttributes.getValue()));

        return createRows(object, indent, toDeletedRowFunc);
    }
}
